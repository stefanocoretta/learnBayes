...
brm_4b_priors <- c(
  ...
  prior(normal(0, 0.1), class = b, coef = `IsWordFALSE:PhonLev`),
  prior(normal(0, 0.1), class = b, coef = `IsWordTRUE:PhonLev`),
...
```{r}
conditional_effects(brm_4b_priorpp, "PhonLev:IsWord")
```
...
```{r}
conditional_effects(brm_4b, "PhonLev:IsWord")
```
